...
Robusta renders Prometheus/resource graphs as SVG charts and rasterizes them
...
def simple_chart_svg() -> bytes:
    chart = XYChart(charts_style(), title="simple chart", width=400, height=300)
    chart.y_labels = [0, 5]
...
def robusta_styled_chart_svg() -> bytes:
    """An SVG built through the real chart pipeline."""
...
def test_convert_robusta_styled_chart_to_png(robusta_styled_chart_svg):
    """This is the closest unit-level reproduction of what every graph-bearing
    alert notification goes through on its way to a chat sink."""
