...
You may see references to "Robusta Classic" in parts of this documentation. Robusta Classic is the original open source alert engine that provides deterministic, rule-based alert enrichment using configurable playbooks. When an alert fires, it automatically attaches relevant context — pod logs, resource state, related events — before routing the enriched alert to notification channels.

Robusta Classic is still included in the Robusta Agent and runs alongside HolmesGPT. It is useful for teams that want predictable, rule-based enrichments in addition to AI-powered investigation. However, HolmesGPT is the primary focus of Robusta's development and the recommended way to investigate alerts.
