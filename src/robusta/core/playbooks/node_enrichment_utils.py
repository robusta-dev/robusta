...
from hikaru.model.rel_1_26 import PodList

from robusta.core.reporting import TableBlock
from robusta.integrations.kubernetes.autogenerated.events import NodeEvent
...
def get_node_status_table_block(node: NodeEvent, table_name: Optional[str] = "*Node status details:*") -> TableBlock:
    """
    Enrich the finding with the node resources available for allocation.
...
def get_node_running_pods_table_block_or_none(
    node: NodeEvent, table_name: Optional[str] = "Pods running on the node"
) -> Optional[TableBlock]:
    """
    Enrich the finding with the node resources available for allocation.
    ...
    return TableBlock(effected_pods_rows, ["namespace", "name", "ready"], table_name="Pods running on the node")
