...
from ...integrations.kubernetes.autogenerated.events import KubernetesAnyChangeEvent, KubernetesResourceEvent
...
class ExamplesGenerator:
    ...
    def get_k8s_trigger_list(cls, name: str, all_triggers, include_prometheus: bool = False):
        if name == "KubernetesAnyChangeEvent" or name == "KubernetesResourceEvent":  # any resource
            ...
            any_triggers.append("Or any other inheriting trigger. See :ref:`Triggers` for details")
            additional_triggers = [trigger for trigger in all_triggers if trigger not in any_triggers]
            any_triggers.extend(
                additional_triggers if len(additional_triggers) < 4 else additional_triggers[0:3]
            )
            any_triggers.append(" * ...")
            return any_triggers
    ...
    def get_supported_triggers(self, action: Action) -> List[str]:
        ...
        all_triggers = [" * " + trigger for trigger in all_triggers]  # create triggers list items
