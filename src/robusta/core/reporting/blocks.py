...
# A single-character ellipsis is used (rather than "...") for table-cell truncation
# so the width accounting stays exact - it consumes exactly one column of the cell.
TABLE_TRUNCATION_ELLIPSIS = "…"
# Never shrink a text column below this many characters, otherwise the value
# becomes unreadable (essentially just the ellipsis).
...
class TableBlock(BaseBlock):
    ...
    def __numeric_column_indices(cls, rendered_rows, num_columns: int) -> set:
        # A column is considered numeric when every non-empty cell parses as a
        # number (e.g. the "Fired"/"Resolved" counters). These are kept at full
        # width so the counts always stay readable.
        numeric_indices = set()
        for idx in range(num_columns):
            values = [str(row[idx]) for row in rendered_rows if idx < len(row)]
    ...
    def __calc_max_width(cls, headers, rendered_rows, table_max_width: int) -> List[int]:
        ...
        columns_max_widths = [len(str(header)) for header in headers]
        ...
        # Only shrink text columns; numeric columns (counters) are left intact so
        # the numbers are never the ones that get truncated.
        ...
        if not shrinkable:  # nothing but numeric columns - fall back to shrinking everything
            shrinkable = list(range(num_columns))

        # Water-fill: repeatedly trim the currently-widest shrinkable column by one
        # character. This distributes the reduction fairly (widest first) instead of
        # gutting a single column, and never drops a column below TABLE_MIN_COLUMN_WIDTH.
        while sum(columns_max_widths) > table_max_width:
            ...
            if not candidates:
                break  # can't shrink any further without making columns unreadable
    ...
    def __truncate_cell(cls, value: str, max_width: int) -> str:
        # Truncate an over-wide cell to a single line with an ellipsis, instead of
        # letting tabulate word-wrap it onto extra physical lines (which mangles
        # every following row in the rendered table).
        ...
        # For dotted, space-free qualified names (e.g. Java class paths like
        # "ats.betting...settler.AbstractBetSettler") the distinctive part is the
        # suffix (the class name), so trim from the left and keep the tail. For any
        # other text a trailing cut reads more naturally.
    ...
    def to_table_string(self, table_max_width: int = PRINTED_TABLE_MAX_WIDTH, table_fmt: str = "presto") -> str:
        ...
        # Truncate over-wide cells ourselves rather than passing maxcolwidths to
        # tabulate. tabulate would *wrap* long cells onto extra physical lines,
        # which corrupts the whole table; truncating keeps every row on one line.
        truncated_headers = [
