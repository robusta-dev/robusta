...
class BaseScopeMatching(ABC):
    @abstractmethod
    def get_scope_matching_data(self) -> Dict[str, Union[str, Dict[str, str]]]:
    ...
    def scope_attribute_matches(self, attr_name: str, attr_matchers: List[str]):
        data = self.get_scope_matching_data()
        ...
        for attr_matcher in attr_matchers:
            if hasattr(self, f"scope_match_{attr_name}"):
                return getattr(self, f"scope_match_{attr_name}")(attr_matcher, attr_value)
