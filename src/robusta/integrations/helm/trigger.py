...
from robusta.integrations.helm.models import HelmAlert, HelmKubernetesAlert
...
class HelmTriggerEvent(TriggerEvent):
    alert: HelmAlert

    def get_event_name(self) -> str:
        return HelmTriggerEvent.__name__

    def get_event_description(self) -> str:
        alert_name = self.alert.labels.get("alertname", "NA")
        alert_severity = self.alert.labels.get("severity", "NA")
        return f"HelmAlert-{alert_name}-{alert_severity}"
...
class HelmAlertTrigger(BaseTrigger):
    """
    ...
    def should_fire(self, event: TriggerEvent, playbook_id: str):
        if not isinstance(event, HelmTriggerEvent):
            return False
    ...
    def build_execution_event(
        self, event: HelmTriggerEvent, sink_findings: Dict[str, List[Finding]]
    ) -> Optional[ExecutionBaseEvent]:
        return AlertEventBuilder.build_event(event, sink_findings)

    @staticmethod
    def get_execution_event_type() -> type:
        return HelmKubernetesAlert


class HelmAlertTriggers(BaseModel):
    on_helm_alert: Optional[HelmAlertTrigger]
...
class AlertEventBuilder:
    ...
    @classmethod
    def __load_node(cls, alert: HelmAlert, node_name: str) -> Optional[Node]:
        node = None
        try:
    ...
    def _build_event_task(
        event: HelmTriggerEvent, sink_findings: Dict[str, List[Finding]]
    ) -> Optional[ExecutionBaseEvent]:
        labels = event.alert.labels
        execution_event = HelmKubernetesAlert(
        ...
        for mapping in MAPPINGS:
            try:
                resource_name = labels.get(mapping.helm_label, None)
        ...
        job_name = labels.get("job", None)  # a helm "job" not a kubernetes "job" resource
