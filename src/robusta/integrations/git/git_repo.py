...
class GitRepo:
    ...
    def init_repo(self):
        with self.repo_lock:
            ...
            self.repo = porcelain.clone(self.git_repo_url, self.repo_local_path, key_filename=self.key_file_name)
    ...
    def push(self):
        with self.repo_lock:
            try:
                ...
                GitRepoManager.remove_git_repo(self.git_repo_url)
                logging.error(f"Push failed {self.repo_local_path}", traceback.print_exc())
                raise e

    def pull_rebase(self):
        with self.repo_lock:
            os.system(f"cd {self.repo_local_path} && GIT_SSH_COMMAND='ssh -i {self.key_file_name} -o IdentitiesOnly=yes' git pull --rebase origin master")
