...
class SeverityMapper:
    @staticmethod
    def get(value: str, default: FindingSeverity = FindingSeverity.INFO) -> FindingSeverity:
        custom_severity_map = GlobalConfig.get_global_config().get("custom_severity_map", {})
        try:
            if value in custom_severity_map.keys():
                return FindingSeverity.from_severity(custom_severity_map[value].upper())
        except:
            logging.exception(f"Failed to map custom severity {value}")
        return SEVERITY_MAP[value] if value in SEVERITY_MAP else default
...
class PrometheusKubernetesAlert(
    ...
    def create_default_finding(self) -> Finding:
        ...
        return Finding(
            ...
            severity=SeverityMapper.get(alert_severity, FindingSeverity.INFO),
