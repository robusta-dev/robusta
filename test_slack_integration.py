#!/usr/bin/env python3
"""
This script is a test harness for the Robusta Slack integration.
It allows you to quickly iterate on Slack message formatting by sending
test messages with mock data.
"""
...
def create_cpu_throttling_finding() -> Finding:
    ...
    finding.add_enrichment([
        MarkdownBlock('📘 *Alert Explanation:* This pod is throttled due to its CPU limit. This can occur even when CPU usage is far below the limit. <https://github.com/robusta-dev/alert-explanations/wiki/CPUThrottlingHigh-(Prometheus-Alert)|Learn more.>'),
        MarkdownBlock("🛠 *Robusta's Recommendation:* Remove this pod's CPU limit entirely. <https://home.robusta.dev/blog/stop-using-cpu-limits/|Using CPU limits is *not* a best practice.>")
