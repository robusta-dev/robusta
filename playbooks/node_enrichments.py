...
def node_running_pods(node_name: str) -> List[BaseBlock]:
    ...
    pod_list: PodList = Pod.listPodForAllNamespaces(field_selector=f"spec.nodeName={node_name}").obj
...
def node_allocatable_resources(node_name: str) -> List[BaseBlock]:
    ...
    block_list: List[BaseBlock] = []
    if node:
        block_list.append(MarkdownBlock("Node Allocatable Resources - The amount of compute resources that are available for pods"))
        block_list.append(TableBlock(node.status.allocatable.items(), ["resource", "value"]))
    return block_list
