...
def restart_loop_reporter(event: PodEvent, config: RestartLoopParams):
    ...
    for container_status in crashed_container_statuses:
        ...
        if container_status.state and container_status.state.waiting:
            event.report_blocks.append(MarkdownBlock(f'*{container_status.name}* waiting reason: {container_status.state.waiting.reason}'))
        if container_status.state and container_status.state.terminated:
            event.report_blocks.append(MarkdownBlock(f'*{container_status.name}* termination reason: {container_status.state.terminated.reason}'))
        if container_status.lastState and container_status.lastState.terminated:
            event.report_blocks.append(MarkdownBlock(f'*{container_status.name}* termination reason: {container_status.lastState.terminated.reason}'))
    ...
    event.report_title = f"Crashing pod {pod.metadata.name} in namespace {pod.metadata.namespace}"
