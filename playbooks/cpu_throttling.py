...
def do_cpu_throttling_analysis(pod: Pod) -> List[BaseBlock]:
    ...
    return [
        MarkdownBlock("*Alert Explanation:* This pod is throttled. It wanted to use the CPU and was blocked due to "
                      "it's CPU limit (<https://github.com/robusta-dev/alert-explanations/wiki/CPUThrottlingHigh-"
                      "(Prometheus-Alert)|learn more)>"),
        MarkdownBlock("_Tip: <https://github.com/robusta-dev/alert-explanations/wiki/CPUThrottlingHigh-(Prometheus-"
                      "Alert)#low-cpu|This can occur even when CPU usage is far below the limit>._"),
        MarkdownBlock("*Robusta's Recommendation:* Remove this pod's CPU limit entirely. <https://github.com/"
